...
import 'package:buddhadham/models/sectionVocab.dart';
...
class ReadScreenForSearchVocab extends StatefulWidget {
  final int initialPage;
  final String searchText;
  const ReadScreenForSearchVocab({Key? key, required this.initialPage, required this.searchText}) : super(key: key);

  @override
  State<ReadScreenForSearchVocab> createState() => _ReadScreenForSearchVocabState();
}

class _ReadScreenForSearchVocabState extends State<ReadScreenForSearchVocab> {
  ...
  Future<List<String>>? getData() async {
    try {
      List<String>? listData = await sectionVocab.listAllText;
      setState(() {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return PageView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                //index = widget.initialPage - 1;
                snapshot.data![index] = snapshot.data![index].replaceAll('<mark>', '');
                snapshot.data![index] = snapshot.data![index].replaceAll('</mark>', '');
                // snapshot.data![index] = snapshot.data![index].replaceAll(widget.searchText, '<mark>${widget.searchText}</mark>');

                final regex = RegExp(r'(<[^>]*>|&nbsp;|' '${widget.searchText}' r')');

                snapshot.data![index] = snapshot.data![index].replaceAllMapped(regex, (match) {
                  if (match.group(0) == widget.searchText) {
                    return '<mark>${widget.searchText}</mark>';
                  } else {
